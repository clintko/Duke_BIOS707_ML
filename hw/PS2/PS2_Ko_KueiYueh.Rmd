...
At 95% level, the p value of testing $E[X^c] = E[X^o]$ is very close to the threshold to reject the null, while comparing $E[X^c | Z > 0] = E[X^o | Z > 0]$ and comparing $E[X^c | Z < 0] = E[X^o | Z < 0]$, we definitely not reject the null since p value is much larger than 0.05. 
...
At 95% level, the p value of testing $E[X^c] = E[X^o]$ and $E[X^c] = E[X^impute]$ are both very close to the threshold to reject the null.
...
```
    ...
    - C. Calculate new imputed mean of X. How does this compare to the true mean of X.
        - The predicted mean is the same as imputed mean. From the t-test, ????
```{r}
cat("", 
    "Mean(x_c):      ", mean(x_c),                  "\n",
    "Mean(x_o):      ", mean(x_o,    na.rm = TRUE), "\n",
...
### initialization
percentage <- seq(from = 0.1, to = 1, by = 0.1)

### simulation
simulation <- function(){
    res <- sapply(percentage, function(p){
        pval <- c()
        x_o  <- x_c
        x_o[sample(idx, size = floor(length(idx) * p))] <- NA
        fit <- lm(x_o ~ z)
        x_pred <- predict(fit, newdata = list(z))

        res_test <- t.test(x_c, x_pred)
        pval <- c(pval, res_test$p.value)
        return(pval)
    }) # end sapply
    return(res)
} # end function

res_pval <- t(replicate(50, simulation()))
colnames(res_pval) <- percentage
res_pval <- res_pval %>% 
    as.data.frame %>% 
    gather(percentage, value) %>% 
    group_by(percentage) %>% 
    summarise(Mean = mean(value))

### show the results
plot(res_pval$percentage, res_pval$Mean)
```
...
mat_cor <- tmp %>% as.data.frame %>% dplyr::select(-date) %>% na.omit %>% as.matrix
...
(d) Plot what you think is the “best” bandwidth for each measurement. Comment
why you chose this value.
...
In my opinion, there is not much pattern for each measurement. That is, the values distributed at the similar mean throughout the time. Therefore, I believe that the bandwidth should be at least above 0.5, since the bandwidth below 0.5 contains unnecessary variations.
